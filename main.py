...
def set_gemma_api_key(e: me.InputBlurEvent):
    me.state(State).gemma_api_key = e.value
...
def model_picker_dialog():
    ...
    with dialog(state.is_model_picker_dialog_open):
        with me.box(style=me.Style(display="flex", flex_direction="column", gap=12)):
            ...
            me.input(
                label="Gemma API Key",
                value=state.gemma_api_key,
                on_blur=set_gemma_api_key,
            )
        ...
        for model in Models:
            if model.name.startswith("GEMINI"):
                ...
                disabled = not state.gemma_api_key
            else:
                disabled = False
            me.checkbox(
                key=model.value,
...
def send_prompt(e: me.ClickEvent):
    ...
    for conversation in state.conversations:
        ...
        elif model == Models.GEMMA_2.value:
            llm_response = gemma.send_prompt(input, history)
        else:
